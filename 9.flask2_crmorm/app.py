...
def home():
    ...
    #     .filter(User.name == "장예진").all()
    
    # for user, order, store in users_order_stores:
    #     print(f'{user.name}이 방문한 상점은 {store.name}, 시간은 {order.orderat}')
 

    users = User.query.filter_by(name="장예진").first() # 여기 all()로 하면 안 됨
    order_by_user = users.orderR
    # 여기에서 "장예진"이 주문한 주문내역
    for order in order_by_user:
        store = order.store
        print(f'장예진이 방문한 상점은 {store.name} 시간은 {order.orderat}')
